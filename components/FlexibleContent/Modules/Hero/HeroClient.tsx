...
type HeroClientProps = HeroSchemaType & { rewardStatsData: RewardStatsDataType };
    ...
    <>
      <div
        className={clsx(
          ...
          variant === 'copyImageStatsHero' ? 'left-0 w-80 lg:absolute lg:top-36' : 'w-full'
        )}
      >
        {title && (
          <AnimatedElement
      ...
      {variant === 'copyImageStatsHero' ? (
        <RewardStats data={rewardStatsData} className={clsx('lg:block', statsHidden && 'hidden')} />
      ) : null}
    </>
  );
}
